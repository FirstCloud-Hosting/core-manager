 ...
 * @version   $Id: 7c90509455515c810067c948e4c14f8de70fda45 $
 */
...
class Link_Environment {
    ...
    public function initializeForm($id, $method, $textSubmit, $textReset, $action=null){
        $this->form[$id]['method'] = $method;
        $this->form[$id]['textSubmit'] = $textSubmit;
        $this->form[$id]['textReset'] = $textReset;
        $this->form[$id]['action'] = $action;
    }

    public function setInputForm($id, $label, $name, $type, $required=False, $autofocus=False, $value=False, $popover=False, $placeholder=False, $readonly=False){
        $this->form[$id]['inputs'][$name] = array(
            'label' => $label,
            'type' => $type,
            'required' => $required,
            'autofocus' => $autofocus,
    ...
    public function setSelectForm($id, $label, $name, $list, $displayName=False, $valueName=False, $selectedValue=False, $required=False, $popover=False, $readonly=False){
        $this->form[$id]['inputs'][$name] = array(
            'label' => $label,
            'type' => 'select',
            'list' => $list,
    ...
    public function setTextareaForm($id, $label, $name, $row, $required=False, $autofocus=False, $value=False, $popover=False, $readonly=False){
        $this->form[$id]['inputs'][$name] = array(
            'label' => $label,
            'type' => 'textarea',
            'row' => $row,
    ...
    public function setCheckboxForm($id, $label, $name, $checked=False, $popover=False, $readonly=False){
        $this->form[$id]['inputs'][$name] = array(
            'label' => $label,
            'type' => 'checkbox',
            'checked' => $checked,
    ...
    public function generateForm($id){
        $method = $this->form[$id]['method'];
        if($this->form[$id]['action'] != null){
            $action = $this->form[$id]['action'];
            echo "<form id='$id' class='form-horizontal' role='form' method='$method' action='$action'>";
        }else{
            echo "<form id='$id' class='form-horizontal' role='form' method='$method'>";
        }

        $textSubmit = $this->form[$id]['textSubmit'];
        $textReset = $this->form[$id]['textReset'];

        if(isset($this->form[$id]['inputs'])){
            foreach ($this->form[$id]['inputs'] as $key => $value) {

                if($value['type'] != 'select' && $value['type'] != 'textarea' && $value['type'] != 'hidden' && $value['type'] != 'checkbox'){
                    $label = $value['label'];
                    $type = $value['type'];
                    ...
                    if($value['autofocus'] == True)
                        $autofocus = 'autofocus';
                    else
                        $autofocus = '';

                    if($value['popover'] != False)
                        $popover = "<i class='fas fa-info-circle' data-toggle='popover' title='$label' data-content='" . $value['popover'] . "'></i>";
                    else
                        $popover = '';

                    if($value['placeholder'] != False)
                    ...
                    if($required == 'required')
                        echo "<label class='col-md-2 control-label'>$label * $popover</label>";
                    else
                        echo "<label class='col-md-2 control-label'>$label $popover</label>";
                    echo '<div class="col-md-12">';
                    if($value['value']){
                        $defaultValue = $value['value'];
                        echo "<input class='form-control' name='$key' type='$type' value='$defaultValue' $required $autofocus $placeholder $readonly>";
                    }else
                        echo "<input class='form-control' name='$key' type='$type' $required $autofocus $placeholder $readonly>";
                    echo '</div>';
                    echo '</div>';
                }elseif($value['type'] == 'hidden'){
                	$defaultValue = $value['value'];
                	echo "<input type='hidden' name='$key' value='$defaultValue'>";                      
                    ...
                    //echo "<select class='form-control selectpicker' data-live-search='true' name='$key' $required $readonly>";
                    echo "<select class='form-control' data-live-search='true' name='$key' $required $readonly>";
                    if($value['valueName'] == False && $value['displayName'] == False){
                        foreach ($value['list'] as $list) {
                            $valueOption = $list;
                            $displayOption = $list;
                            if($value['selected'] && $value['selected'] == $valueOption)
                                echo "<option selected value='$valueOption'>$displayOption</option>";
                            else
                                echo "<option value='$valueOption'>$displayOption</option>";
                        }
                    }else{
                        foreach ($value['list'] as $list) {
                            $valueOption = $list[$value['valueName']];
                            $displayOption = $list[$value['displayName']];
                            if($value['selected'] && $value['selected'] == $valueOption)
                                echo "<option selected value='$valueOption'>$displayOption</option>";
                            else
                                echo "<option value='$valueOption'>$displayOption</option>";
                    ...
                    echo '<div class="form-group">';
                    echo "<label class='col-md-5 control-label'>$label $popover</label>";
                    echo '<div class="toggle">';
                    echo "<label class='col-md-5 control-label'>";
                    echo "<input class='form-control' id='$key' name='$key' type='checkbox' $checked $readonly><span class='button-indecator'></span>";
                    echo '</label>';
                    echo '</div>';
                    echo '</div>';
                }else{
                    ...
                    echo '<div class="col-sm-12">';
                    echo "<textarea class='form-control' name='$key' rows='$row' $required $autofocus $readonly>$defaultValue</textarea>";
                    echo '</select>';
                    echo '</div>';
                    echo '</div>';
                }
        ...
        echo '<input type="hidden" name="' .  "token_" . $_SESSION['randomTokenName'] . '" value="' . $_SESSION['token'] . '">';

        echo "<center><button type='submit' class='btn btn-primary'>$textSubmit</button><button type='reset' class='btn btn-default'>$textReset</button></center>";
